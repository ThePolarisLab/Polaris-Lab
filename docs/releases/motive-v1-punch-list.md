...
- Subsequent controlled schema capture returned `top_level_type=object`, `top_level_keys=["pagination","vehicle_idle_rollups"]`, `item_container_key=vehicle_idle_rollups`, `item_count_observed=0`, and `pagination_keys=["page_no","per_page","total"]`. The production envelope is partially verified, but the previous fixture collection key `vehicle_utilization` is not production-certified for success responses and item identity/period/metric/unit schema remains unknown because zero items were returned.
- The bounded schema experiment uses up to three existing organization-owned stored vehicles in one provider request solely to obtain a non-empty sanitized `vehicle_idle_rollups` item schema; it does not enable or certify utilization ingestion.
