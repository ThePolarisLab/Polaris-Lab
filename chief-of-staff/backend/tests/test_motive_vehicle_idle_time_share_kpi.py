...
ORG_A = "org-idle-kpi-a"
ORG_B = "org-idle-kpi-b"
...
def test_ratio_of_sums_and_coverage_are_observed_vehicle_day_based(kpi_session):
    ...
    # 100% row + 0% row would average to 50%, but ratio-of-sums is 10%.
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START, idle_time=Decimal("100"), driving_time=Decimal("0"))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[1], day=WINDOW_START, idle_time=Decimal("0"), driving_time=Decimal("900"))
    # Returned 0/0 is a provider rollup but not metric-valid.
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START + timedelta(days=1), idle_time=Decimal("0"), driving_time=Decimal("0"))
    # Null is incomplete.
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[1], day=WINDOW_START + timedelta(days=1), idle_time=None, driving_time=Decimal("50"))
    # Non-current provenance is a returned rollup, not metric-valid.
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START + timedelta(days=2), idle_time=Decimal("50"), driving_time=Decimal("50"), metric_units=True)
    # Multi-day and out-of-window rows do not count as certified one-day rollups.
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[1], day=WINDOW_START + timedelta(days=2), end_day=WINDOW_START + timedelta(days=3), idle_time=Decimal("50"), driving_time=Decimal("50"))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START - timedelta(days=1), idle_time=Decimal("50"), driving_time=Decimal("50"))

    result = vehicle_idle_time_share_kpi(db, ORG_A)
    ...
    assert result == {
        ...
        "idle_time_metric_coverage_percent": 14.29,
...
def test_returned_zero_idle_is_real_zero_percent(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("0"), driving_time=Decimal("600"))
...
def test_returned_zero_driving_is_real_100_percent(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("600"), driving_time=Decimal("0"))
...
def test_zero_total_time_is_unavailable_not_synthetic_zero(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("0"), driving_time=Decimal("0"))
    ...
    assert result["idle_time_metric_coverage_percent"] == 0.0


def test_null_time_is_incomplete_not_zero(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=None, driving_time=Decimal("100"))
...
@pytest.mark.parametrize(
    ("idle_time", "driving_time"),
    [(Decimal("-1"), Decimal("100")), (Decimal("100"), Decimal("-1"))],
)
def test_negative_time_fails_closed(kpi_session, idle_time, driving_time):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=idle_time, driving_time=driving_time)
    ...
    assert result["value_percent"] is None
    assert result["provider_rollup_vehicle_days"] == 1
    assert result["idle_time_metric_coverage_percent"] is None
...
def test_full_metric_coverage_is_fleet_representative(kpi_session):
    ...
    for offset in range(PRODUCTION_HORIZON_DAYS):
        _rollup(
            ...
            idle_time=Decimal("25"),
            driving_time=Decimal("75"),
        )
    ...
    assert result["idle_time_metric_coverage_percent"] == 100.0
...
def test_latest_failed_production_state_makes_kpi_unavailable(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("20"), driving_time=Decimal("80"))
...
def test_cross_tenant_rows_and_history_cannot_influence_result(kpi_session):
    ...
    for offset in range(PRODUCTION_HORIZON_DAYS):
        day = WINDOW_START + timedelta(days=offset)
        _rollup(db, organization_id=ORG_A, vehicle=vehicle_a, day=day, idle_time=Decimal("10"), driving_time=Decimal("90"))
        _rollup(db, organization_id=ORG_B, vehicle=vehicle_b, day=day, idle_time=Decimal("90"), driving_time=Decimal("10"))
...
def test_response_is_sanitized(kpi_session):
    ...
    assert "provider-org-idle-kpi-a-1" not in serialized
    assert "run-org-idle-kpi-a" not in serialized
    ...
    try:
        ...
        for offset in range(PRODUCTION_HORIZON_DAYS):
            _rollup(
                ...
                vehicle=vehicle,
                day=WINDOW_START + timedelta(days=offset),
                idle_time=Decimal("20"),
                driving_time=Decimal("80"),
            )

        def forbidden_provider_call(*_args, **_kwargs):
            raise AssertionError("read-only idle-time-share endpoint must not call Motive")
        ...
        response = client.get("/api/v1/motive/fleet/vehicle-idle-time-share-kpi", headers=headers)

        assert response.status_code == 200
        body = response.json()
        assert body["status"] == "available_observed"
        assert body["value_percent"] == 20.0
        assert body["fleet_representative"] is True
        assert body["time_unit"] == "seconds"
