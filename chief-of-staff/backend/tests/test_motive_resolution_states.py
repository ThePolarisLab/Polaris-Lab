...
def row(states):
    return maintenance_readiness_for_trucks(
        truck_numbers=["M2214"], as_of_date=DAY, connector=C(states)
    )["classifications"][0]
...
def test_true_first_open_is_preserved_across_repeated_open_observations():
    g = row(
        [
            ("2026-09-07", "open"),
            ("2026-09-08", "open"),
    ...
    )["inspection_resolution_groups"][0]
    assert g["first_open_date"] == "2026-09-07"
    assert g["resolution_state"] == "open"


def test_good_only_category_is_omitted_from_resolution_history():
    result = row([("2026-09-12", "good")])
    assert result["inspection_resolution_groups"] == []
    assert result["explicitly_resolved_inspection_issue_count"] == 0
    assert result["reopened_inspection_issue_count"] == 0
