...
ORG_A = "org-total-burn-kpi-a"
ORG_B = "org-total-burn-kpi-b"
...
def test_ratio_of_sums_converts_combined_seconds_to_observed_hours_and_reports_coverage(kpi_session):
    db, _engine = kpi_session
    vehicles = _seed_healthy_org(db, ORG_A, 2)

    # Per-row blended rates are 2.0 and 1.0 gal/observed-hour. Their simple
    # mean would be 1.50, but the required ratio of sums is 5 gal / 4 h = 1.25.
    _rollup(
        db,
        organization_id=ORG_A,
        vehicle=vehicles[0],
        day=WINDOW_START,
        idle_time=Decimal("1800"),
        driving_time=Decimal("1800"),
        idle_fuel=Decimal("0.5"),
        driving_fuel=Decimal("1.5"),
    )
    _rollup(
        db,
        organization_id=ORG_A,
        ...
        day=WINDOW_START,
        idle_time=Decimal("0"),
        driving_time=Decimal("10800"),
        idle_fuel=Decimal("0"),
        driving_fuel=Decimal("3"),
    )
    _rollup(
        db,
        organization_id=ORG_A,
    ...
    _rollup(
        ...
        day=WINDOW_START + timedelta(days=1),
        idle_time=None,
        driving_time=Decimal("3600"),
        idle_fuel=Decimal("0"),
        driving_fuel=Decimal("1"),
    )
    _rollup(
        db,
        organization_id=ORG_A,
        ...
        idle_time=Decimal("3600"),
        driving_time=Decimal("3600"),
        idle_fuel=Decimal("1"),
        driving_fuel=Decimal("9"),
        metric_units=True,
    )
    _rollup(
        db,
    ...
    result = vehicle_fuel_burn_rate_kpi(db, ORG_A)

    assert result == {
        "status": "available_observed",
        "kpi": "observed_7_day_fuel_burn_rate",
        "window_start": WINDOW_START.isoformat(),
        "window_end": WINDOW_END.isoformat(),
        "request_timezone": "America/Chicago",
        "value_gallons_per_observed_hour": 1.25,
        ...
        "fuel_burn_rate_metric_coverage_percent": 14.29,
...
def test_positive_observed_time_with_zero_total_fuel_is_real_zero_rate(kpi_session):
    db, _engine = kpi_session
    ...
    assert result["value_gallons_per_observed_hour"] == 0.0
...
def test_zero_time_zero_fuel_has_no_burn_rate_denominator(kpi_session):
    ...
    assert result["value_gallons_per_observed_hour"] is None
    assert result["provider_rollup_vehicle_days"] == 1
    assert result["metric_valid_vehicle_days"] == 0
    assert result["fuel_burn_rate_metric_coverage_percent"] == 0.0


def test_zero_total_time_with_positive_fuel_fails_closed(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(
        db,
    ...
    assert result["value_gallons_per_observed_hour"] is None
    assert result["provider_rollup_vehicle_days"] == 1
    assert result["fuel_burn_rate_metric_coverage_percent"] is None


@pytest.mark.parametrize(
    ("idle_time", "driving_time", "idle_fuel", "driving_fuel"),
    [
        (Decimal("-1"), Decimal("3600"), Decimal("0"), Decimal("1")),
        (Decimal("0"), Decimal("-1"), Decimal("0"), Decimal("1")),
        (Decimal("0"), Decimal("3600"), Decimal("-1"), Decimal("1")),
        (Decimal("0"), Decimal("3600"), Decimal("0"), Decimal("-1")),
    ],
)
def test_negative_component_fails_closed(kpi_session, idle_time, driving_time, idle_fuel, driving_fuel):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(
        db,
...
@pytest.mark.parametrize("missing_field", ["idle_time", "driving_time", "idle_fuel", "driving_fuel"])
def test_null_component_is_incomplete_not_zero(kpi_session, missing_field):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    values = {
        "idle_time": Decimal("1800"),
    ...
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, **values)
    result = vehicle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["status"] == "unavailable"
    assert result["value_gallons_per_observed_hour"] is None
    assert result["metric_valid_vehicle_days"] == 0
...
def test_full_metric_coverage_is_fleet_representative(kpi_session):
    ...
    for offset in range(PRODUCTION_HORIZON_DAYS):
        _rollup(
            ...
            driving_time=Decimal("3600"),
            idle_fuel=Decimal("0.5"),
            driving_fuel=Decimal("7.5"),
        )
    result = vehicle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["value_gallons_per_observed_hour"] == 4.0
    assert result["provider_rollup_coverage_percent"] == 100.0
    assert result["fuel_burn_rate_metric_coverage_percent"] == 100.0
...
def test_response_is_sanitized_and_does_not_expose_raw_totals(kpi_session):
    ...
    assert "provider-org-total-burn-kpi-a-1" not in serialized
    assert "run-org-total-burn-kpi-a" not in serialized
    for forbidden_key in (
        ...
        "api_key",
        "token",
        "observed_operating_seconds",
        "observed_fuel_gallons",
        "distance",
        "engine_hours",
    ):
    ...
    try:
        ...
        for offset in range(PRODUCTION_HORIZON_DAYS):
            _rollup(
                ...
                day=WINDOW_START + timedelta(days=offset),
                idle_time=Decimal("3600"),
                driving_time=Decimal("7200"),
                idle_fuel=Decimal("1"),
                driving_fuel=Decimal("17"),
            )

        def forbidden_provider_call(*_args, **_kwargs):
            raise AssertionError("read-only total-fuel-burn-rate endpoint must not call Motive")
        ...
        response = client.get("/api/v1/motive/fleet/vehicle-fuel-burn-rate-kpi", headers=headers)
        assert response.status_code == 200
        body = response.json()
        assert body["status"] == "available_observed"
        assert body["kpi"] == "observed_7_day_fuel_burn_rate"
        assert body["value_gallons_per_observed_hour"] == 6.0
        ...
        assert body["rate_unit"] == "gallons_per_observed_hour"
