...
ORG_A = "org-idle-burn-kpi-a"
ORG_B = "org-idle-burn-kpi-b"
...
def test_ratio_of_sums_converts_seconds_to_idle_hours_and_reports_coverage(kpi_session):
    db, _engine = kpi_session
    vehicles = _seed_healthy_org(db, ORG_A, 2)

    # Row burn rates are 2.0 and 1.333... gal/idle-hour. Their simple mean
    # would be about 1.67, but the required ratio of sums is 3 gal / 2 h = 1.50.
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START, idle_time=Decimal("1800"), idle_fuel=Decimal("1"))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[1], day=WINDOW_START, idle_time=Decimal("5400"), idle_fuel=Decimal("2"))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START + timedelta(days=1), idle_time=Decimal("0"), idle_fuel=Decimal("0"))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[1], day=WINDOW_START + timedelta(days=1), idle_time=None, idle_fuel=Decimal("1"))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START + timedelta(days=2), idle_time=Decimal("3600"), idle_fuel=Decimal("9"), metric_units=True)
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[1], day=WINDOW_START + timedelta(days=2), idle_time=Decimal("3600"), idle_fuel=Decimal("9"), end_day=WINDOW_START + timedelta(days=3))
    _rollup(db, organization_id=ORG_A, vehicle=vehicles[0], day=WINDOW_START - timedelta(days=1), idle_time=Decimal("3600"), idle_fuel=Decimal("9"))

    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    ...
    assert result == {
        ...
        "value_gallons_per_idle_hour": 1.5,
        ...
        "idle_fuel_burn_rate_metric_coverage_percent": 14.29,
...
def test_positive_idle_time_with_zero_fuel_is_real_zero_rate(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("3600"), idle_fuel=Decimal("0"))
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["status"] == "available_observed"
    assert result["value_gallons_per_idle_hour"] == 0.0
...
def test_zero_time_zero_fuel_has_no_burn_rate_denominator(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("0"), idle_fuel=Decimal("0"))
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["status"] == "unavailable"
    assert result["value_gallons_per_idle_hour"] is None
    assert result["provider_rollup_vehicle_days"] == 1
    assert result["metric_valid_vehicle_days"] == 0
    assert result["idle_fuel_burn_rate_metric_coverage_percent"] == 0.0


def test_zero_idle_time_with_positive_fuel_fails_closed(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("0"), idle_fuel=Decimal("1"))
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["status"] == "unavailable"
    assert result["value_gallons_per_idle_hour"] is None
    assert result["provider_rollup_vehicle_days"] == 1
    assert result["idle_fuel_burn_rate_metric_coverage_percent"] is None
...
def test_negative_idle_metric_fails_closed(kpi_session, idle_time, idle_fuel):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=idle_time, idle_fuel=idle_fuel)
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["status"] == "unavailable"
    assert result["idle_fuel_burn_rate_metric_coverage_percent"] is None


def test_null_idle_metric_is_incomplete_not_zero(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=None, idle_fuel=Decimal("1"))
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["status"] == "unavailable"
    assert result["value_gallons_per_idle_hour"] is None
    assert result["metric_valid_vehicle_days"] == 0
...
def test_full_metric_coverage_is_fleet_representative(kpi_session):
    ...
    for offset in range(PRODUCTION_HORIZON_DAYS):
        _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START + timedelta(days=offset), idle_time=Decimal("3600"), idle_fuel=Decimal("1.25"))
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["value_gallons_per_idle_hour"] == 1.25
    assert result["provider_rollup_coverage_percent"] == 100.0
    assert result["idle_fuel_burn_rate_metric_coverage_percent"] == 100.0
...
def test_latest_failed_production_state_makes_kpi_unavailable(kpi_session):
    db, _engine = kpi_session
    vehicle = _seed_healthy_org(db, ORG_A, 1)[0]
    _rollup(db, organization_id=ORG_A, vehicle=vehicle, day=WINDOW_START, idle_time=Decimal("3600"), idle_fuel=Decimal("1"))
...
def test_cross_tenant_rows_cannot_influence_result(kpi_session):
    ...
    for offset in range(PRODUCTION_HORIZON_DAYS):
        ...
        _rollup(db, organization_id=ORG_A, vehicle=vehicle_a, day=day, idle_time=Decimal("3600"), idle_fuel=Decimal("1"))
        _rollup(db, organization_id=ORG_B, vehicle=vehicle_b, day=day, idle_time=Decimal("3600"), idle_fuel=Decimal("9"))
    result = vehicle_idle_fuel_burn_rate_kpi(db, ORG_A)
    assert result["value_gallons_per_idle_hour"] == 1.0
...
def test_response_is_sanitized_and_does_not_expose_raw_totals(kpi_session):
    ...
    assert "provider-org-idle-burn-kpi-a-1" not in serialized
    assert "run-org-idle-burn-kpi-a" not in serialized
...
def test_endpoint_accepts_connector_read_and_makes_no_provider_call(monkeypatch):
    ...
    try:
        ...
        for offset in range(PRODUCTION_HORIZON_DAYS):
            _rollup(db, organization_id=organization["id"], vehicle=vehicle, day=WINDOW_START + timedelta(days=offset), idle_time=Decimal("7200"), idle_fuel=Decimal("3"))

        def forbidden_provider_call(*_args, **_kwargs):
            raise AssertionError("read-only idle-fuel-burn-rate endpoint must not call Motive")
        ...
        response = client.get("/api/v1/motive/fleet/vehicle-idle-fuel-burn-rate-kpi", headers=headers)
        assert response.status_code == 200
        body = response.json()
        assert body["status"] == "available_observed"
        assert body["kpi"] == "observed_7_day_idle_fuel_burn_rate"
        assert body["value_gallons_per_idle_hour"] == 1.5
        assert body["fleet_representative"] is True
        assert body["idle_time_unit"] == "seconds"
        assert body["fuel_unit"] == "gallons"
        assert body["rate_unit"] == "gallons_per_idle_hour"
