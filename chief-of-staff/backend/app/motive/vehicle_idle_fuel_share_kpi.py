"""Read-only observed Motive vehicle idle-fuel-share business KPI.
...
``docs/engineering/MOTIVE_IDLE_FUEL_SHARE_THIRD_BUSINESS_KPI_DESIGN.md``.
"""
...
KPI_NAME = "observed_7_day_vehicle_idle_fuel_share"
_STATUS_AVAILABLE = "available_observed"
_STATUS_UNAVAILABLE = "unavailable"
_PERCENT_QUANTUM = Decimal("0.01")


def vehicle_idle_fuel_share_kpi(session: Session, organization_id: str) -> dict[str, Any]:
    """Return the sanitized tenant-scoped observed idle-fuel-share KPI."""
    try:
        return _vehicle_idle_fuel_share_kpi(session, organization_id)
    except Exception:
        return _unavailable()


def _vehicle_idle_fuel_share_kpi(session: Session, organization_id: str) -> dict[str, Any]:
    ...
    # Production history persists only the selected count, not the exact
    # historical provider-ID population. Reuse the conservative guard from
    # the certified utilization and idle-time-share KPIs rather than guessing.
    current_vehicle_ids = [
        row[0]
        for row in (
    ...
    observed_idle_fuel = Decimal("0")
    observed_driving_fuel = Decimal("0")
    metric_valid_vehicle_days = 0

    for row in by_identity.values():
        if row.metric_units is not False:
            continue
        if row.idle_fuel is None or row.driving_fuel is None:
            continue

        idle_fuel = _decimal(row.idle_fuel)
        driving_fuel = _decimal(row.driving_fuel)
        if idle_fuel is None or driving_fuel is None or idle_fuel < 0 or driving_fuel < 0:
        ...
        total_fuel = idle_fuel + driving_fuel
        if total_fuel <= 0:
            # Returned 0/0 is real data but has no positive denominator for a
            # fuel-share calculation; it remains incomplete for this KPI.
            continue

        observed_idle_fuel += idle_fuel
        observed_driving_fuel += driving_fuel
    ...
    observed_total_fuel = observed_idle_fuel + observed_driving_fuel

    if metric_valid_vehicle_days == 0 or observed_total_fuel <= 0:
        return _unavailable(
            window_start=window_start,
            window_end=window_end,
            selected_vehicle_count=selected_vehicle_count,
            ...
            missing_requested_vehicle_days=missing_requested_vehicle_days,
            provider_rollup_coverage_percent=provider_coverage,
            idle_fuel_metric_coverage_percent=metric_coverage,
        )

    value_percent = _quantize((observed_idle_fuel * Decimal("100")) / observed_total_fuel)
    return _payload(
...
def _unavailable_with_provider_coverage(
    *,
    ...
    return _unavailable(
        ...
        idle_fuel_metric_coverage_percent=None,
    )
...
def _payload(
    *,
    ...
    idle_fuel_metric_coverage_percent: float | None,
    ...
    return {
        ...
        "missing_requested_vehicle_days": missing_requested_vehicle_days,
        "provider_rollup_coverage_percent": provider_rollup_coverage_percent,
        "idle_fuel_metric_coverage_percent": idle_fuel_metric_coverage_percent,
        "fleet_representative": fleet_representative,
...
def _unavailable(
    *,
    ...
    idle_fuel_metric_coverage_percent: float | None = None,
) -> dict[str, Any]:
    return _payload(
