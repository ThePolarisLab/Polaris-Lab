"""Read-only observed Motive driving-fuel burn-rate business KPI.
...
``docs/engineering/MOTIVE_DRIVING_FUEL_BURN_RATE_FIFTH_BUSINESS_KPI_DESIGN.md``.
"""
...
KPI_NAME = "observed_7_day_driving_fuel_burn_rate"
_STATUS_AVAILABLE = "available_observed"
_STATUS_UNAVAILABLE = "unavailable"
_RATE_QUANTUM = Decimal("0.01")
_DRIVING_TIME_UNIT = "seconds"
_RATE_UNIT = "gallons_per_driving_hour"
_SECONDS_PER_HOUR = Decimal("3600")


def vehicle_driving_fuel_burn_rate_kpi(session: Session, organization_id: str) -> dict[str, Any]:
    """Return the sanitized tenant-scoped observed driving-fuel burn-rate KPI."""
    try:
        return _vehicle_driving_fuel_burn_rate_kpi(session, organization_id)
    except Exception:
        return _unavailable()


def _vehicle_driving_fuel_burn_rate_kpi(session: Session, organization_id: str) -> dict[str, Any]:
    ...
    observed_driving_fuel_gallons = Decimal("0")
    metric_valid_vehicle_days = 0

    for row in by_identity.values():
        if row.metric_units is not False:
            continue
        if row.driving_time is None or row.driving_fuel is None:
            continue

        driving_time = _decimal(row.driving_time)
        driving_fuel = _decimal(row.driving_fuel)
        if driving_time is None or driving_fuel is None or driving_time < 0 or driving_fuel < 0:
        ...
        if driving_time == 0:
            ...
            # A real 0-second / 0-gallon provider observation has no burn-rate
            # denominator. Keep it in provider coverage but not metric coverage.
            continue

        observed_driving_seconds += driving_time
        observed_driving_fuel_gallons += driving_fuel
    ...
    if metric_valid_vehicle_days == 0 or observed_driving_seconds <= 0:
        return _unavailable(
            window_start=window_start,
            window_end=window_end,
            selected_vehicle_count=selected_vehicle_count,
            ...
            missing_requested_vehicle_days=missing_requested_vehicle_days,
            provider_rollup_coverage_percent=provider_coverage,
            driving_fuel_burn_rate_metric_coverage_percent=metric_coverage,
        )

    value = _quantize((observed_driving_fuel_gallons * _SECONDS_PER_HOUR) / observed_driving_seconds)
    return _payload(
        status=_STATUS_AVAILABLE,
        window_start=window_start,
        window_end=window_end,
        value_gallons_per_driving_hour=value,
        selected_vehicle_count=selected_vehicle_count,
        expected_requested_vehicle_days=expected_vehicle_days,
        provider_rollup_vehicle_days=provider_rollup_vehicle_days,
        metric_valid_vehicle_days=metric_valid_vehicle_days,
...
def _unavailable_with_provider_coverage(
    *,
    ...
    return _unavailable(
        ...
        driving_fuel_burn_rate_metric_coverage_percent=None,
    )
...
def _payload(
    *,
    ...
    value_gallons_per_driving_hour: float | None,
    ...
    driving_fuel_burn_rate_metric_coverage_percent: float | None,
    ...
    return {
        ...
        "value_gallons_per_driving_hour": value_gallons_per_driving_hour,
        "selected_vehicle_count": selected_vehicle_count,
        "expected_requested_vehicle_days": expected_requested_vehicle_days,
        "provider_rollup_vehicle_days": provider_rollup_vehicle_days,
        "metric_valid_vehicle_days": metric_valid_vehicle_days,
        "missing_requested_vehicle_days": missing_requested_vehicle_days,
        "provider_rollup_coverage_percent": provider_rollup_coverage_percent,
        "driving_fuel_burn_rate_metric_coverage_percent": driving_fuel_burn_rate_metric_coverage_percent,
        "fleet_representative": fleet_representative,
...
def _unavailable(
    *,
    ...
    driving_fuel_burn_rate_metric_coverage_percent: float | None = None,
) -> dict[str, Any]:
    return _payload(
        status=_STATUS_UNAVAILABLE,
        window_start=window_start,
        window_end=window_end,
        value_gallons_per_driving_hour=None,
        selected_vehicle_count=selected_vehicle_count,
        expected_requested_vehicle_days=expected_requested_vehicle_days,
        provider_rollup_vehicle_days=provider_rollup_vehicle_days,
        metric_valid_vehicle_days=metric_valid_vehicle_days,
