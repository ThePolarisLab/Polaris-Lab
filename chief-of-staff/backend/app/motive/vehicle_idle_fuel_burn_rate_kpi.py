"""Read-only observed Motive idle-fuel burn-rate business KPI.
...
``docs/engineering/MOTIVE_IDLE_FUEL_BURN_RATE_FOURTH_BUSINESS_KPI_DESIGN.md``.
"""
...
KPI_NAME = "observed_7_day_idle_fuel_burn_rate"
_STATUS_AVAILABLE = "available_observed"
_STATUS_UNAVAILABLE = "unavailable"
_RATE_QUANTUM = Decimal("0.01")
_IDLE_TIME_UNIT = "seconds"
_RATE_UNIT = "gallons_per_idle_hour"
_SECONDS_PER_HOUR = Decimal("3600")


def vehicle_idle_fuel_burn_rate_kpi(session: Session, organization_id: str) -> dict[str, Any]:
    """Return the sanitized tenant-scoped observed idle-fuel burn-rate KPI."""
    try:
        return _vehicle_idle_fuel_burn_rate_kpi(session, organization_id)
    except Exception:
        return _unavailable()


def _vehicle_idle_fuel_burn_rate_kpi(session: Session, organization_id: str) -> dict[str, Any]:
    ...
    observed_idle_fuel_gallons = Decimal("0")
    metric_valid_vehicle_days = 0

    for row in by_identity.values():
        if row.metric_units is not False:
            continue
        if row.idle_time is None or row.idle_fuel is None:
            continue

        idle_time = _decimal(row.idle_time)
        idle_fuel = _decimal(row.idle_fuel)
        if idle_time is None or idle_fuel is None or idle_time < 0 or idle_fuel < 0:
        ...
        observed_idle_seconds += idle_time
        observed_idle_fuel_gallons += idle_fuel
    ...
    if metric_valid_vehicle_days == 0 or observed_idle_seconds <= 0:
        return _unavailable(
            window_start=window_start,
            window_end=window_end,
            selected_vehicle_count=selected_vehicle_count,
            ...
            missing_requested_vehicle_days=missing_requested_vehicle_days,
            provider_rollup_coverage_percent=provider_coverage,
            idle_fuel_burn_rate_metric_coverage_percent=metric_coverage,
        )

    value = _quantize((observed_idle_fuel_gallons * _SECONDS_PER_HOUR) / observed_idle_seconds)
    return _payload(
        status=_STATUS_AVAILABLE,
        window_start=window_start,
        window_end=window_end,
        value_gallons_per_idle_hour=value,
        selected_vehicle_count=selected_vehicle_count,
        expected_requested_vehicle_days=expected_vehicle_days,
        provider_rollup_vehicle_days=provider_rollup_vehicle_days,
        metric_valid_vehicle_days=metric_valid_vehicle_days,
...
def _unavailable_with_provider_coverage(
    *,
    ...
    return _unavailable(
        ...
        idle_fuel_burn_rate_metric_coverage_percent=None,
    )
...
def _payload(
    *,
    ...
    value_gallons_per_idle_hour: float | None,
    ...
    idle_fuel_burn_rate_metric_coverage_percent: float | None,
    ...
    return {
        ...
        "value_gallons_per_idle_hour": value_gallons_per_idle_hour,
        "selected_vehicle_count": selected_vehicle_count,
        "expected_requested_vehicle_days": expected_requested_vehicle_days,
        "provider_rollup_vehicle_days": provider_rollup_vehicle_days,
        "metric_valid_vehicle_days": metric_valid_vehicle_days,
        "missing_requested_vehicle_days": missing_requested_vehicle_days,
        "provider_rollup_coverage_percent": provider_rollup_coverage_percent,
        "idle_fuel_burn_rate_metric_coverage_percent": idle_fuel_burn_rate_metric_coverage_percent,
        "fleet_representative": fleet_representative,
...
def _unavailable(
    *,
    ...
    idle_fuel_burn_rate_metric_coverage_percent: float | None = None,
) -> dict[str, Any]:
    return _payload(
        status=_STATUS_UNAVAILABLE,
        window_start=window_start,
        window_end=window_end,
        value_gallons_per_idle_hour=None,
        selected_vehicle_count=selected_vehicle_count,
        expected_requested_vehicle_days=expected_requested_vehicle_days,
        provider_rollup_vehicle_days=provider_rollup_vehicle_days,
        metric_valid_vehicle_days=metric_valid_vehicle_days,
