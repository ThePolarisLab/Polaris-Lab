...
def motive_vehicle_utilization_writer_contract_status(db: Session, organization_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "request_window_identity": {
            ...
            "scope": "Polaris-owned idempotency key for returned, validated rollups from an explicit completed request window.",
            "prefers_internal_vehicle_fk": True,
            "migration_required_now": False,
            "reason": "The key is safe as a Polaris replay boundary for explicit request windows; it is not a provider-returned reporting-period identity.",
        },
        ...
        "unit_policy": {
            ...
            "reason": "Fuel metrics must not be combined unless unit context is known and consistent.",
        },
