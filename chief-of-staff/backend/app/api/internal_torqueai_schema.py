"""Machine-authenticated, read-only TorqueAI certification endpoints."""
...
from app.connectors.torqueai_schema import dispatch_schema_paths, dispatch_stop_job_values
...
def _configured_connector() -> TorqueAIConnector:
    organization_slug = str(os.getenv(TORQUEAI_ORGANIZATION_SLUG_ENV) or "").strip()
    return TorqueAIConnector(organization_slug=organization_slug)
...
async def certify_torqueai_schema_machine(
    ...
    _verify_empty_machine_request(
        ...
        body_error_detail="TorqueAI schema certification body must be empty",
    )
...
@router.post("/stop-job-certification")
async def certify_torqueai_stop_jobs_machine(
    request: Request,
    certification_date: date = Query(..., alias="date"),
    x_polaris_job_timestamp: str | None = Header(default=None, alias="X-Polaris-Job-Timestamp"),
    x_polaris_job_signature: str | None = Header(default=None, alias="X-Polaris-Job-Signature"),
):
    """Observe only the bounded categorical values of ``stops[].job``."""
    body = await request.body()
    _verify_empty_machine_request(
        request=request,
        body=body,
        timestamp_header=x_polaris_job_timestamp,
        signature_header=x_polaris_job_signature,
        body_error_detail="TorqueAI stop job certification body must be empty",
    )
    ...
    try:
        ...
        observed_job_values = dispatch_stop_job_values(page.data)
    except TorqueAIConnectorError as exc:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail={
        ...
        raise HTTPException(
            ...
            detail={
                "status": "failed",
                "provider": "torqueai",
                "error_code": "unsafe_stop_job_category_shape",
                "raw_dispatches_returned": False,
                "secrets_exposed": False,
            },
        ) from exc
    ...
    return {
        "status": "certified_categorical_values_observed",
        "provider": "torqueai",
        "operation": "external_dispatch_stop_job_values",
        "request": {
            "from": certification_date.isoformat(),
            "to": certification_date.isoformat(),
            "page": 1,
        ...
        "rows_returned": len(page.data),
        "total_count": page.total_count,
        "certified_field": "stops[].job",
        "observed_job_values": list(observed_job_values),
        "only_certified_field_values_returned": True,
        "raw_dispatches_returned": False,
        "secrets_exposed": False,
    }
