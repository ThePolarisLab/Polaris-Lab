...
export function ConnectorsView() {
  ...
  async function syncMotiveUsers() {
    setBusy("motive-users");
    setError("");
    setNotice(null);
    try {
      await apiClient.post("/api/v1/motive/sync/users");
      await load();
      setNotice({ tone: "success", message: "Motive company-user sync completed. Driver classification is not certified." });
    } catch (requestError) {
      const fallback = requestError.status === 401 ? "Motive Company API Key authorization is required." : requestError.status === 429 ? "Motive user sync is rate limited; Retry-After is honored when present." : "Motive user sync failed safely.";
      setError(requestError.message || fallback);
      await load();
    } finally {
      setBusy("");
